...
class ArpsCiscoAsa( Arps ):
    ...
    def _get(self):
        # get a list of teh vlan numbers first - assume dot notation on interface names
        interface_to_vlan = {}
        for d in self.prompt.tell_and_match( 'show ip address', r'^\S+\.(?P<vlan>\d+)\s+(?P<interface>\S+)\s+'):
            interface_to_vlan[d['interface']] = int(d['vlan'])
            
        logging.error("MAPPING: %s" % (interface_to_vlan,))
        
        for m in self.matches.keys():
            for d in self.prompt.tell_and_match( m, self.matches[m] ):
                if d['interface'] in interface_to_vlan:
                    d['vlan'] = interface_to_vlan[ d['interface'] ]
